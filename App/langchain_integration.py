...
def generate_job_listing(job_position, tech_stack, company_name, company_values):
    prompt = PromptTemplate(
        ...
        template="You are an expert job listing creator.Generate a job listing (USING HTML CODE) for a {job_position} at {company_name} "
                 "using {tech_stack}"
                 " and following company values: {company_values}."
    )
